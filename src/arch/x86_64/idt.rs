...
lazy_static! {
    static ref IDT: Idt = {
        ...
        // Interrupts
        idt.set_interrupt(1, handler!(keyboard_handler));
...
impl Idt {
    ...
    pub fn set_interrupt(&mut self, vector: u8, func: unsafe extern "C" fn()) {
        self.0[(vector + 0x20) as usize] = IdtEntry::new(func);
    }
...
// PIC vector 7
extern "C" fn spurious_handler(frame: &InterruptFrame) {
    // Do not send EOI
}
